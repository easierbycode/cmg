...
// The azlegend checkout is expected at ../azlegend (override: AZLEGEND_DIR).
...
const azlegendRoot = Deno.env.get("AZLEGEND_DIR")
  ...
  : new URL("../../../azlegend/static", import.meta.url).pathname;
...
Deno.test("sound.js replacement loads and plays all 6 boss BGMs through the embedded player", async () => {
  assert(
    (await Deno.stat(`${azlegendRoot}/player.html`).catch(() => null))?.isFile,
    `azlegend player not found at ${azlegendRoot} — set AZLEGEND_DIR`,
  );
