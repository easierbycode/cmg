...
export function validateTutorial(raw: unknown): Tutorial {
  ...
  if (typeof t.description !== "string") {
    throw new Error("tutorial: missing description");
  }
  ...
  if (t.steps) {
    ...
    for (const s of t.steps) {
      if (!names.has(s.file)) {
        throw new Error(`tutorial: step targets unknown file ${s.file}`);
      }
      if (typeof s.insert !== "string") {
        throw new Error("tutorial: step.insert must be a string");
      }
    }
  }
  return t;
