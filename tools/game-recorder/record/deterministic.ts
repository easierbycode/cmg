...
export async function recordDeterministic(
  ...
  try {
    ...
    while (tick < maxTicks && saved < wantedTicks && tailLeft !== 0) {
      await page.evaluate("window.__cmgStep()");
      tick++;
      const dataUrl = await page.evaluate("window.__cmgFrame()") as
        | string
        | null;
      if (!dataUrl) {
        throw new Error(
          `scene "${spec.name}": no canvas on the page to capture`,
        );
      }
      ...
      if (tailLeft > 0) tailLeft--;
        ...
        tailLeft < 0 && spec.stopWhen && await page.evaluate(spec.stopWhen)
      ) {
        tailLeft = tailTicks;
      }
    }
