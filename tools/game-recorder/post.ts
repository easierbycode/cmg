// CLI: deno task record:post --platform instagram --video out/racer-intro.reel.mp4
//      deno task record:post --platform tiktok    --video out/racer-intro.reel.mp4
//
...
import { postReel } from "./social/instagram.ts";
import { postVideo } from "./social/tiktok.ts";
import { serveViaTunnel } from "./social/tunnel.ts";
...
Usage: deno task record:post --platform instagram|tiktok --video <reel.mp4> [options]
  --video <file>      the transcoded 9:16 mp4 (from deno task record)
...
const OUT_DIR = resolve(Deno.env.get("RECORDINGS_DIR") ?? "out/recordings");
const underOut = (p: string): string => {
  const abs = resolve(p);
  if (abs !== OUT_DIR && !abs.startsWith(OUT_DIR + SEPARATOR)) {
    console.error(
      `refusing ${p} — only files under ${OUT_DIR} may be published`,
    );
    Deno.exit(1);
  }
  return abs;
...
if (platform === "instagram") {
  ...
  if (!videoUrl) {
    ...
    if (args["dry-run"]) {
      console.log(
        `[dry-run] would tunnel ${args.video}${
          cover ? ` + ${cover}` : ""
        } via ngrok and publish a Reel`,
      );
      Deno.exit(0);
    }
  ...
  try {
    ...
    console.log(
      `  published: media ${mediaId}${permalink ? ` — ${permalink}` : ""}`,
    );
  } finally {
    await tunnel?.close();
  }
  ...
  if (args["dry-run"]) {
    console.log(
      `[dry-run] would direct-post ${args.video} to TikTok as ${privacy}`,
    );
    Deno.exit(0);
  }
  ...
  console.error(
    `unknown platform "${args.platform}" — use instagram or tiktok`,
  );
  Deno.exit(1);
}
