...
export async function serveViaTunnel(
  ...
  try {
    ...
    throw new Error(
      `couldn't start ngrok (${
        (e as Error).message
      }) — install it (https://ngrok.com/download) ` +
  ...
  const publicUrl = await (async () => {
    ...
    const reader = ngrok.stdout.pipeThrough(new TextDecoderStream())
      .getReader();
    let buf = "";
    try {
  ...
  console.log(
    `serving ${
      Object.values(files).map((f) => basename(f)).join(", ")
