...
async function api(
  ...
  if (!res.ok || (error && error.code !== "ok")) {
    throw new Error(
      `TikTok API ${res.status} ${error?.code ?? ""}: ${
...
export async function postVideo(
  ...
  if (allowed.length && !allowed.includes(privacy)) {
    throw new Error(
      `privacy level ${privacy} not available for this account (allowed: ${
        allowed.join(", ")
      }) — ` +
  ...
  try {
    for (let i = 0; i < chunkCount; i++) {
      ...
      const end = i === chunkCount - 1
        ? size
        : Math.min(start + chunkSize, size);
      ...
      if (!res.ok && res.status !== 201) {
        throw new Error(
          `chunk ${i + 1}/${chunkCount} upload failed: HTTP ${res.status}`,
        );
      }
      console.log(
        `  uploaded chunk ${i + 1}/${chunkCount} (${end - start} bytes)`,
      );
    }
  } finally {
    file.close();
