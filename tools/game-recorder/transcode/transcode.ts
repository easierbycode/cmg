 ...
 */
const FIT_SHARP = "scale=iw*2:ih*2:flags=neighbor," +
  "scale={W}:{H}:force_original_aspect_ratio=decrease:flags=lanczos";

function filterGraph(
  o: Required<
    Pick<ReelOptions, "width" | "height" | "fill" | "background" | "fps">
  >,
) {
  const fit = FIT_SHARP.replaceAll("{W}", String(o.width)).replaceAll(
    "{H}",
    String(o.height),
  );
  if (o.fill === "blur") {
